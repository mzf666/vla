...
需要说明的是第三类：**工作取值（tentative）**。像模型规模区间、压缩后的成功率预算、脱机时长这些，我们给了具体数字，是为了让方案可讨论、可反驳——工程上没有具体数字就没法开始。但它们是随测量结果移动的工作取值，不是承诺。文中凡是这一类的数字都会写明它是工作取值，读的时候直接当作"当前的起点，后面会改"就对了 [computed: 本文的取数规则]。
...
验收标准是任务成功率的差值，不是任何代理指标。压缩悬崖的位置已经公开：4 bpw 几乎免费（96.6%），3 bpw 到 94.8%，2.5 bpw 是拐点（85.7%），2 bpw 直接崩到 48.0% [[arXiv:2605.24011]](https://arxiv.org/abs/2605.24011)。最后一步就丢掉 37.7 pt [computed: 85.7 − 48.0]，所以我们的起步规则是**停在 4 bpw** [[arXiv:2605.24011]](https://arxiv.org/abs/2605.24011)。

这里要把话说准：4 bpw 那 0.4 pt 是**量化这一步单独的代价** [[arXiv:2605.24011]](https://arxiv.org/abs/2605.24011)，不是整条链路的代价。蒸馏、剪枝、编译各自还有自己的 delta，而且精度损失是会累加的——我们在速度那一侧已经说过不能把各步收益相乘，精度这一侧同样不能假装它们不叠加 [[arXiv:2604.24447]](https://arxiv.org/abs/2604.24447)。所以 0.4 pt 我们只当作整条链路的**起步预算**，P3 逐步测出每一步的实际代价之后重新分配。链路总预算是多少，现在给不出来，它挂在第 7 部分。
...
把未知点名，并给每一项配上关掉它的那个实验——这比处处自信更值得相信。以下九项是我们目前明确知道自己不知道的 [computed: 全文未解决项的汇总]。
...
- **整条压缩链路的成功率总预算。** 公开数据只覆盖单步代价，链路各步的 delta 如何叠加没有公开结果 [[arXiv:2604.24447]](https://arxiv.org/abs/2604.24447)。由 P3 逐步实测后分配。
